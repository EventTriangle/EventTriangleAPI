...
public class AzureAdService : IAzureAdService
{
    ...
    public async Task<AzureAdAuthorizationDataResponse> GetAuthorizationData(string code, string codeVerifier)
    {
        ...
        var result = JsonConvert.DeserializeObject<AzureAdAuthorizationDataResponse>(json, jsonSerializerSettings);
        
        return result;
    }
    
    public async Task<AzureAdAuthorizationDataResponse> GetRefreshData(string code, string codeVerifier)
    {
        var appSettingsPath = _appSettingsService.GetAppSettingsPathSender();
        ...
        var httpRequest = new HttpRequestMessage(HttpMethod.Post, url) { Content = new FormUrlEncodedContent(dict) };
